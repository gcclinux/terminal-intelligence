...
# Install on Linux/macOS: https://aka.ms/powershell
```
...
Ctrl+W → navigate to project folder → Select Current Directory
```
...
mkdir ~/.ti/archive
mv ~/.ti/session_token_chat_2026*.md ~/.ti/archive/
```
...
# Restore manually
cp important-file.py.backup important-file.py
```
...
# Add to PATH
sudo ln -s ~/path/to/ti-darwin-arm64 /usr/local/bin/ti
```
...
# Detach: Ctrl+B, D
# Reattach: tmux attach -t ti-session
```
...
**Remember:**
- Press `Ctrl+H` anytime for quick reference
- Use `/help` for command list
- Experiment in a test workspace first
...
- [AWS Bedrock](https://aws.amazon.com/bedrock/) - Enterprise AI
