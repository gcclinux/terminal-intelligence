...
func TestKeyboard_TabSwitchesPanes(t *testing.T) {
	...
	t.Run("tab switches from AI to editor", func(t *testing.T) {
		...
		// Verify switched to editor pane
		if app.GetActivePane() != types.EditorPaneType {
			t.Errorf("Expected focus on editor pane after tab, got %v", app.GetActivePane())
		}
	})

	t.Run("multiple tab presses toggle between panes", func(t *testing.T) {
		app := ui.New(nil)
		...
		for i := 0; i < 4; i++ {
			...
			// Odd presses should be AI, even should be editor
			expectedPane := types.AIPaneType
			if i%2 == 1 {
				expectedPane = types.EditorPaneType
			}
			
			if app.GetActivePane() != expectedPane {
				t.Errorf("After %d tab presses, expected %v, got %v", i+1, expectedPane, app.GetActivePane())
			}
		}
	})
}
